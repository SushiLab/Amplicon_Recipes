...
then
    ...
    awk -F '.' '/^>/ {print $1}' $output_f/unclassified.fa | sort -V | uniq -c | awk 'NR==1{split($0,cols,"\t")}; NR>FNR{sub(">", "", $0); counts[$2]=$1}; END{printf "Unclassified"; for(col=1; col<=length(cols); col++){if(cols[col] in counts){printf "\t"counts[cols[col]]}else{printf "\t0"}}}' $output_f/otutab_initial_classified.txt - >> $output_f/otutab_initial_classified.txt
...
then
    ...
    # APPEND unclassified counts CURRENTLY ONLY FOR .TXT FILE, OTHER FORMATS ARE NOT NICE
    awk -F '.' '/^>/ {print $1}' $output_f/still_unclassified.fa | sort -V | uniq -c | awk 'NR==1{split($0,cols,"\t")}; NR>FNR{sub(">", "", $0); counts[$2]=$1}; END{printf "Unclassified"; for(col=1; col<=length(cols); col++){if(cols[col] in counts){printf "\t"counts[cols[col]]}else{printf "\t0"}}}' $output_f/otutab_final_classified.txt - >> $output_f/otutab_final_classified.txt
    echo -e "\n...done quantifying reads.\n"
fi
